...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Position : MMF_Feedback
	{
		...
		public enum Modes { AtoB, AlongCurve, ToDestination, ToDestinationAlongCurve }
		...
		[MMFEnumCondition("Mode", (int)Modes.AtoB, (int)Modes.ToDestination, (int)Modes.ToDestinationAlongCurve)]
				...
				switch (Mode)
				{
					case Modes.ToDestination:
						...
						_coroutine = Owner
							.StartCoroutine(
								IsDynamic 
								? MoveFromTo(AnimatePositionTarget, _initialPosition, DestinationPositionTransform, FeedbackDuration, AnimatePositionTween) 
								: MoveFromTo(AnimatePositionTarget, _initialPosition, _destinationPosition, FeedbackDuration, AnimatePositionTween));
					...
					case Modes.AlongCurve:
						...
						break;
					case Modes.ToDestinationAlongCurve:
						if (DestinationPositionTransform != null)
						{
							_destinationPosition = GetPosition(DestinationPositionTransform);
						}
						...
						_coroutine = Owner.StartCoroutine(
							ToDestinationAlongCurve(AnimatePositionTarget.transform.position, _destinationPosition));
		...
		protected virtual IEnumerator MoveFromTo(GameObject movingObject, Vector3 pointA, Transform pointB, float duration, MMTweenType tweenType)
		{
			IsPlaying = true;
			float journey = NormalPlayDirection ? 0f : duration;
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
			{
				...
				_newPosition = Vector3.LerpUnclamped(pointA, pointB.position, curveValue);
				SetPosition(movingObject.transform, _newPosition);
			...
			if (NormalPlayDirection)
			{
				SetPosition(movingObject.transform, pointB.position);    
			}
			else
			{
				SetPosition(movingObject.transform, pointA);
