...
namespace Gameplay.Lumberjack
{
    public class LumberjackStorage
    {
        ...
        public bool IsFull => ResourceDropouts.Count >= _storageCapacity;

        public LumberjackStorage(LumberjackBotConfig lumberjackBotConfig, Transform lootStackBottom)
        {
            _lootStackBottom = lootStackBottom;
            
            _storageCapacity = lumberjackBotConfig.InitialStorageCapacity;
            _lootPositionOffset = lumberjackBotConfig.LootPositionOffset;
            _storageFloors = lumberjackBotConfig.StorageFloors;

            _resourceCells = new Transform[_storageCapacity];
        }
        ...
        {
            if (_freePositionIndex < _storageCapacity)
        ...
        {
            for (int index = 0; index < _storageCapacity; index++)
            {
